...
public class PostServico {
  ...
  public CadastroPostResponse cadastrarPost(UUID usuarioID, CadastrarPostRequest request) {
    ...
    return new CadastroPostResponse(novoPost.getId(), novoPost.getUser().getId(),
  ...
  public Update atualizarPost(UUID postId, CadastrarPostRequest request) {
  ...
  public List<PostDto> filtrarPostsPorEstado(State state) {
    return postRepo.findByItemEstado(state).stream().map(PostDto::new).toList();
  }
  ...
  public ExcluirPostResponse excluirPostPorId(UUID idPost, UUID idUsuario) {
    ...
    return new ExcluirPostResponse(postParaRemover.getCreatedAt(),
  ...
  public EncerrarProcuraResponse encerrarProcuraDeItem(EncerrarProcuraRequest request) {
    ...
    postParaEncerrar.setType(Type.RETURNED);
    postParaEncerrar.setReturnedAt(LocalDate.now());
    this.postRepo.save(postParaEncerrar);

    itemParaEncerrar.setState(State.RETURNED);
    this.itemRepo.save(itemParaEncerrar);

    return new EncerrarProcuraResponse(postParaEncerrar.getUser().getId(),
